    ...
    protected
      def make_image(env, headers = {}, status = 404)
        ...
        if Utils::simple_captcha_value(code)
          ...
          path_to_image = generate_simple_captcha_image(code)
          send_file(path_to_image, :type => 'image/jpeg', :disposition => 'inline', :filename =>  'simple_captcha.jpg')
        else
          [status, headers, body]
        end
      end
      ...
      def refresh_code(env)
        ...
        body = %Q{
                    $(".simple_captcha_image img").attr('src', '#{url}');
                    $("#check_captcha_key").attr('value', '#{key}');
                  }
